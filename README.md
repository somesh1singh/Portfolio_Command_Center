...
The three core inputs remain unchanged. Optional advanced inputs add opening holdings, mutual-fund transactions, bank-paid investment income, broker Tax P&L, and a verified corporate-action register.

## Features

- Imports CSV, XLSX, and XLS trade ledgers
- Provides separate Tradebook, Funds Ledger, and Current Holdings upload fields
- Supports one or more Zerodha-account exports in each core upload field; identical rows are de-duplicated only within the same source account, while current holdings are consolidated for the portfolio view
- Mutual-fund transaction exports, dividend/investment-income statements, and corporate-action registers also accept multiple files per session
- Shows equity and mutual-fund holdings separately, with one-based serial numbers and compact Indian rupee display (₹12.35 L / ₹1.23 Cr; no paise)
...
## Advanced completeness inputs

- **Opening holdings:** positions and average costs immediately before the first tradebook date; converted into opening FIFO lots.
- **Mutual-fund transactions:** date, scheme, purchase/redemption type, units, and NAV/amount; combined with the transaction engine for scheme-level analysis.
- **Bank-paid dividends/investment income:** externally received income added to economic return and account XIRR after row-level confirmation.
- **Broker Tax P&L:** retained as an independent reconciliation source beside the app's indicative FIFO tax classification.
- **Corporate-action register:** verified actions with columns `Date`, `Type`, `Old Symbol`, `New Symbol`, `Numerator`, `Denominator`, `Cost Allocation %`, and optional `Source/Notes`.
...
Yahoo split checking is a candidate finder, not an authoritative corporate-action source. Verify every candidate against an NSE/BSE filing or company announcement. Bonus, merger and demerger cost allocation normally require the company's official shareholder tax notice and must be entered manually. Dividends remain cash-flow entries in the funds ledger and are never treated as quantity adjustments.
...
- Verified ISIN overrides are included for INA, GREENPOWER, GREENPLY, SIYARAM-M, and DENISCHEM-X; the override list remains visible in `app.py` for audit and future updates.
...
- The app labels holdings-plus-cash less net bank funding as an **account reconciliation difference**, not automatically as investment profit/loss. Dividends paid directly to the linked bank and other off-platform proceeds must be added separately for complete economic return.
