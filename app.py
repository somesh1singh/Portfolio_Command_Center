...
with st.sidebar:
    ...
    with st.expander("Advanced completeness inputs"):
        ...
        income_upload = st.file_uploader("F. Bank-paid dividends/investment income", type=["csv", "xlsx", "xls"], key="investment_income", help="Dividend or investment-income rows paid outside Zerodha.")
        tax_upload = st.file_uploader("G. Broker Tax P&L", type=["csv", "xlsx", "xls"], key="tax_pnl", help="Used as an independent realised-gain reconciliation source.")
...
if tax_upload:
    try:
        tax_pnl_df = read_upload(tax_upload)
        st.success(f"Broker Tax P&L loaded for independent reconciliation: {len(tax_pnl_df):,} rows.")
    except Exception as exc:
        st.warning(f"Broker Tax P&L could not be read: {exc}")
...
with tab2:
    ...
    if not tax_pnl_df.empty:
        st.markdown("**Uploaded broker Tax P&L — independent source**")
        st.dataframe(tax_pnl_df, use_container_width=True)
        st.download_button("Download normalized broker Tax P&L", tax_pnl_df.to_csv(index=False).encode(), "broker_tax_pnl_source.csv", "text/csv")
