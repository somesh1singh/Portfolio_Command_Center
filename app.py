...
def analytics_style(frame, rupee_columns=(), pnl_columns=("Realized P&L", "Unrealized P&L", "Total P&L")):
    ...
    available_money = [c for c in rupee_columns if c in frame.columns]
...
mf_tx = pd.DataFrame()
if mf_upload:
    st.subheader("3. Confirm mutual-fund transaction columns")
    try:
        mf_source = read_upload_bundle(mf_upload)
        mf_detected = identify_columns(mf_source)
        mf_options = ["— Not available —"] + list(mf_source.columns)
        mf_mapping = {}
        mf_cols = st.columns(6)
        for i, field in enumerate(["date", "symbol", "type", "quantity", "price", "amount"]):
            default = mf_detected.get(field, mf_options[0])
            mf_mapping[field] = mf_cols[i].selectbox(field.title(), mf_options, index=mf_options.index(default), key=f"mf_map_{field}")
            if mf_mapping[field] == mf_options[0]:
                mf_mapping[field] = None
...
st.subheader("4. Corporate actions")
...
st.info("No register is required to start. The app can fetch Yahoo split candidates below; confirm bonuses, mergers and demerger cost allocations from NSE/BSE or the company announcement before applying them. Official corporate-action notices: https://www.nseindia.com/companies-listing/corporate-filings-actions")
...
if st.checkbox("Check Yahoo Finance for split ratio candidates", value=True):
...
if ledger_upload:
    st.subheader("5. Confirm funds-ledger columns")
    try:
...
if income_upload:
    st.subheader("6. Confirm bank-paid investment income")
    try:
        income_source = read_upload_bundle(income_upload)
        income_options = ["— Not available —"] + list(income_source.columns)
        income_detected = identify_columns(income_source)
        desc_candidates = identify_ledger_columns(income_source)
        i1, i2, i3, i4 = st.columns(4)
        income_date = i1.selectbox("Income date", income_options, index=income_options.index(income_detected.get("date", income_options[0])), key="income_date")
        income_symbol = i2.selectbox("Income symbol (optional)", income_options, index=income_options.index(income_detected.get("symbol", income_options[0])), key="income_symbol")
        income_desc = i3.selectbox("Income description", income_options, index=income_options.index(desc_candidates.get("description", income_options[0])), key="income_desc")
        income_amount = i4.selectbox("Income amount/credit", income_options, index=income_options.index(income_detected.get("amount", desc_candidates.get("credit", income_options[0]))), key="income_amount")
        if income_date != income_options[0] and income_amount != income_options[0]:
            income_df = pd.DataFrame({
                ...
                "Symbol": income_source[income_symbol].astype(str).str.upper() if income_symbol != income_options[0] else "UNALLOCATED",
                "Description": income_source[income_desc].astype(str) if income_desc != income_options[0] else "Investment income",
...
if holdings_upload:
    st.subheader("7. Confirm current-holdings columns")
    try:
...
with tab2:
    ...
    if not broker_pnl_summary.empty:
        ...
        with st.expander("View Zerodha symbol-level P&L"):
            st.dataframe(numbered(tax_pnl_df), hide_index=True, use_container_width=True)
        with st.expander("View Zerodha other debits and credits"):
            st.dataframe(numbered(broker_pnl_other), hide_index=True, use_container_width=True)
...
with tab4:
    ...
    if reconciliation.empty:
        ...
        st.dataframe(numbered(reconciliation), hide_index=True, use_container_width=True)
