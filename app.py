...
if opening_upload:
    try:
        ...
        synthetic = pd.DataFrame({"date": opening_date, "symbol": opening_positions["Symbol"], "original_symbol": opening_positions["Symbol"], "isin": opening_positions["ISIN"], "quantity": opening_positions["Quantity"], "price": opening_positions["Average Cost"], "amount": opening_positions["Cost Value"]})
...
with tab2:
    ...
    stock_display = stock_report.sort_values("Total Return on Purchases %", ascending=False)
    ...
    duration_display = holding_report.sort_values("Weighted Holding Days", ascending=False).copy()
    ...
    if closed_trades.empty:
        ...
        closed_display["Holding Days"] = closed_display["Holding Days"].round().astype("Int64")
        ...
        if export_format == "CSV":
            ...
            st.download_button("Download filtered closed-trade Excel", excel_report_bytes({"Closed Trades": filtered_closed}), "closed_trade_analytics.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        else:
    ...
    if monthly_profit.empty:
        ...
        st.dataframe(analytics_style(numbered(monthly_profit), ["Realized P&L"]), hide_index=True, use_container_width=True)
        ...
        month_stocks = month_lots.groupby("Symbol", as_index=False).agg(**{"Realized P&L": ("Realized P&L", "sum"), "Closed Lots": ("Symbol", "size"), "Average Return %": ("Return %", "mean")}).sort_values("Realized P&L", ascending=False)
        st.dataframe(analytics_style(numbered(month_stocks), ["Realized P&L"]), hide_index=True, use_container_width=True)
...
with tab3:
    ...
    if closed_trades.empty:
        ...
        heat_month_stocks = heat[heat["Exit Month"].eq(heat_month)].groupby("Symbol", as_index=False).agg(**{"Realized P&L": ("Realized P&L", "sum"), "Closed Lots": ("Symbol", "size"), "Average Return %": ("Return %", "mean")}).sort_values("Realized P&L", ascending=False)
        st.dataframe(analytics_style(numbered(heat_month_stocks), ["Realized P&L"]), hide_index=True, use_container_width=True)
