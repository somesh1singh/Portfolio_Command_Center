...
for asset_class in asset_order + [x for x in asset_values.unique() if x not in asset_order]:
    ...
    reviewed = st.data_editor(
        ...
        disabled=["S.No.", "Symbol", "Quantity", "Average Cost", "Cost Value", "Realized P&L", "Yahoo Ticker", "Uploaded Price", "Uploaded Market Value", "Yahoo Price", "Yahoo Difference %", "Price Source"],
        column_config={"Current Price": st.column_config.NumberColumn(min_value=0.0, format="₹%.0f"), "Yahoo Difference %": st.column_config.NumberColumn(format="%.2f%%")},
    )
...
with tab2:
    ...
    if funds_monthly.empty:
        ...
        monthly_with_total = with_total_row(funds_monthly, "Month")
        st.dataframe(analytics_style(numbered(monthly_with_total), [c for c in ["Funds Added", "Funds Withdrawn", "Charges/Taxes", "Dividends", "External Income", "Net Funds Added"] if c in monthly_with_total]), hide_index=True, use_container_width=True)
        ...
        yearly_with_total = with_total_row(funds_yearly, "Year")
        annual_money_columns = [c for c in ["Funds Added", "Funds Withdrawn", "Charges/Taxes", "Dividends", "External Income", "Net Funds Added"] if c in yearly_with_total]
        st.dataframe(analytics_style(numbered(yearly_with_total), annual_money_columns), hide_index=True, use_container_width=True)
    ...
    activity_detail = tx.copy()
    activity_detail["Trade Month"] = activity_detail["date"].dt.to_period("M").astype(str)
    activity_detail["Trade Year"] = activity_detail["date"].dt.year
    activity_by_stock = activity_detail.groupby(["Trade Year", "Symbol"], as_index=False).agg(Transactions=("symbol", "size"), Active_Months=("Trade Month", "nunique"), Buys=("quantity", lambda x: int((x > 0).sum())), Sells=("quantity", lambda x: int((x < 0).sum()))).rename(columns={"Active_Months": "Active Months"})
    selected_activity_year = st.selectbox("Trading activity year", sorted(activity_by_stock["Trade Year"].unique().tolist()), key="activity_year")
    year_stock_activity = activity_by_stock[activity_by_stock["Trade Year"].eq(selected_activity_year)].sort_values(["Transactions", "Active Months"], ascending=False)
